...
st.set_page_config(
    page_title="Linear Regression - Santé Plus",
    page_icon="📈",
    layout="wide"
)
...
with col_title:
    ...
    <div style="margin-top: 0.5rem;">
        <h1 style="color: #1f77b4; margin-bottom: 0.5rem;">📈 Linear Regression</h1>
    </div>
...
# =========================
st.markdown("""
<div style="background: linear-gradient(135deg, #ff7f0e 0%, #ffbb78 100%); 
            padding: 1.5rem; border-radius: 12px; color: white; margin-bottom: 2rem;">
    <p style="font-size: 1.05rem; line-height: 1.8; margin: 0;">
        Régression linéaire utilisée comme classifieur via un seuil de décision (0.5).
        <br>Ce modèle simple mais efficace transforme la sortie continue en classification binaire.
    </p>
</div>
""", unsafe_allow_html=True)
...
def load_models():
    ...
    return lin_reg, scaler

lin_reg, scaler = load_models()
...
# =========================
if submitted:
    # Construction du tableau d'entrée dans l'ordre correct
    X_input = np.array([[
    ...
    # Standardisation
    X_input_std = scaler.transform(X_input)

    # Prédiction continue
    score = lin_reg.predict(X_input_std)[0]
    ...
    # Affichage du score
    st.metric("Score de prédiction", f"{score:.4f}", f"Seuil: 0.5")
